...
namespace App1_NossoChat.Service
{
    public class ServiceWS
    {
        ...
        public static Usuario GetUsario(Usuario usuario)
        {
            var URL = EnderecoBase + "/usuario";

            //QueryString
            //StringContent parametro = new StringContent(string.Format("?nome={0}&password={1}",usuario.nome,usuario.password));

            FormUrlEncodedContent parametro = new FormUrlEncodedContent(new[] {
                new KeyValuePair<string,string>("nome",usuario.nome),
                new KeyValuePair<string,string>("password",usuario.password),
                });

            HttpClient requisicao = new HttpClient();
            HttpResponseMessage resposta = requisicao.PostAsync(URL, parametro).GetAwaiter().GetResult();
        ...
        public static bool RenomearChat(Chat chat)
        {
            ...
            HttpClient requisicao = new HttpClient();
            HttpResponseMessage resposta = requisicao.PutAsync(URL, parametro).GetAwaiter().GetResult();
        ...
        public static List<Mensagem> GetMensagems(Chat chat)
        {
